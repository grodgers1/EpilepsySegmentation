...
class Center2D(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels, deconv_channels, dropout, up_mode):
        super(Center2D, self).__init__()

        assert (up_mode == 'trans' ) | (up_mode == 'bilinear'), 'up_mode must be trans or bilinear'
        ...
        if (up_mode == 'trans'):
            ...
            layers = [
                ...
                nn.Conv2d(out_channels, deconv_channels, kernel_size=3, padding=1),
                nn.ReLU(inplace=True)
            ]
...
class Last2D(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels, softmax=True):
